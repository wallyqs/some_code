...
void NormalKeys(unsigned char key, int x, int y){
  ...
  if (key == 'h')  {      scene[globalSelection]->translateXaxis(1.0f) ; glutPostRedisplay(); }
  if (key == 'j')  {      scene[globalSelection]->translateYaxis(1.0f) ; glutPostRedisplay(); }
  if (key == 'k')  {      scene[globalSelection]->translateZaxis(1.0f) ; glutPostRedisplay(); }


  if (key == 'y')  {      scene[globalSelection]->translateXaxis(-1.0f) ; glutPostRedisplay(); }
  if (key == 'u')  {      scene[globalSelection]->translateYaxis(-1.0f) ; glutPostRedisplay(); }
  if (key == 'i')  {      scene[globalSelection]->translateZaxis(-1.0f) ; glutPostRedisplay(); }
  if (key == 'n')  {      scene[globalSelection]->paintOrigin(); glutPostRedisplay(); }
...
{
  if(key == GLUT_KEY_UP)
    scene[globalSelection]->moveSceneXaxis(-5.0f);

  if(key == GLUT_KEY_DOWN)
    scene[globalSelection]->moveSceneXaxis(5.0f);

  if(key == GLUT_KEY_LEFT)
    scene[globalSelection]->moveSceneYaxis(5.0f);
